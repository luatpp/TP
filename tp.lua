...
-- ═══════════════════════════════════════════════
-- SECURITY — Proteção Máxima / Alto Desempenho
-- ═══════════════════════════════════════════════
do
    local _LP  = game:GetService("Players").LocalPlayer
    local _ts  = tostring

    -- Snapshot de funções originais (imutável)
    ...
    local _af  = appendfile or function() end
    local _gmt = getmetatable

    -- Pre-computar tostrings (evita chamar tostring toda vez)
    local _lsStr  = _ts(_ls)
    local _pcStr  = _ts(_pc)
    local _rgStr  = _ts(_rg)
    local _reqStr = _ts(_req)
    local _prStr  = _ts(_pr)
    local _wfStr  = _ts(_wf)
    local _afStr  = _ts(_af)

    -- Função de kick otimizada
    local function kick(r)
        _pc(function() _LP:Kick("[SEC] "..r) end)
    end

    -- Writefile filtrado (salvo só XiUtils)
    local function wfSafe(n, d)
        if n and _ts(n):find("XiUtils") then return _wf(n, d) end
    end
    if writefile then writefile = wfSafe end
    if appendfile then appendfile = function() end end

    -- Remover debug hooks imediatamente
    _pc(function() if debug and debug.sethook then debug.sethook() end end)

    -- Lista de spy vars pré-compilada (sem recriar toda iteração)
    local SPY_VARS = {
        "SPY_ACTIVE","HTTP_SPY","LS_HOOK","HOOK_ACTIVE","hookLS",
        "hookloadstring","spyActive","httpSpy","loadstringSpy",
        "ls_original","PASSIVE_SPY","passiveSpy","scriptSpy",
        "codeSpy","reqHook","requestHook","lsHook","writefileSpy",
        "captureLS","captureLoadstring","hookReq","origLS","__spy__",
        "hookHttpGet","HttpSpy","getHttp","interceptHttp","grabCode"
    }
    local SPY_COUNT = #SPY_VARS

    -- Obter genv uma vez
    local _genv = (getgenv and getgenv()) or _G

    -- Contador para rotacionar verificações pesadas (não rodar tudo toda vez)
    local _tick = 0

    task.spawn(function()
        while task.wait(5) do -- 5s: menos impacto no FPS que 3s
            _tick = _tick + 1

            -- Verificações leves (toda iteração)
            if _ts(loadstring) ~= _lsStr then kick("LS hook"); break end
            if _ts(pcall)      ~= _pcStr  then kick("pcall hook"); break end

            -- Verificações médias (toda iteração)
            if _ts(rawget)   ~= _rgStr  then kick("rawget hook"); break end
            if _ts(require)  ~= _reqStr then kick("require hook"); break end
            if _ts(print)    ~= _prStr  then print = _pr end -- restaurar silencioso

            -- Anti-spy: iterar lista pré-compilada
            for i = 1, SPY_COUNT do
                if _genv[SPY_VARS[i]] then kick("Spy: "..SPY_VARS[i]); break end
            end

            -- Verificações pesadas rotacionadas (a cada 3 ciclos = 15s)
            if _tick % 3 == 0 then
                -- Anti-debug hook
                _pc(function() if debug and debug.sethook then debug.sethook() end end)
                ...
                _pc(function()
                    local mt = _gmt(game)
                    if mt and _rg(mt,"__newindex") then _rs(mt,"__newindex",nil) end
                end)

                -- Re-bloquear writefile se foi restaurado
                if writefile and _ts(writefile) == _wfStr then
                    writefile = wfSafe
                end

                -- Re-bloquear appendfile se foi restaurado
                if appendfile and _ts(appendfile) == _afStr then
                    appendfile = function() end
                end

                -- Anti-environment injection
                if getfenv then
                    local env = getfenv(0)
                    if env and (env["__spy__"] or env["_hookActive"]) then
                        kick("Env injection"); break
                    end
                end

                -- Resetar genv (pode ter sido trocado)
                _genv = (getgenv and getgenv()) or _G
            end
        end
    end)
end
